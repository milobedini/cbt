...
export const startAttempt = async (req: Request, res: Response) => {
  try {
    ...
    if (assignmentId) {
      ...
      assignment = await findActiveAssignment(
        userId,
        mod._id as Types.ObjectId,
      );
      if (!assignment) {
        res.status(403).json({
          success: false,
    ...
    if (mod.type === "general_goals") {
      ...
      if (prevGoalsList?.length) {
        ...
        attempt.generalGoals = {
          goals: prevGoalsList.map((g: { goalText?: string }) => ({
            goalText: g.goalText,
            rating: null,
          })),
          reflection: "",
  ...
  try {
    ...
    if (attempt.moduleType === "five_areas_model") {
      ...
      if (fiveAreas) {
        const existing =
          (
            attempt.fiveAreas as unknown as {
      ...
      if (Array.isArray(diaryEntries)) {
        ...
        if (
          ...
          const existing = new Map(
            (attempt.diaryEntries as unknown as Record<string, unknown>[]).map(
              (e) => [key(e), e],
            ),
          );
          ...
          attempt.diaryEntries =
            sanitized as unknown as typeof attempt.diaryEntries;
        }
      }
...
export const submitAttempt = async (req: Request, res: Response) => {
  try {
    ...
    if (attempt.moduleType === "reading") {
      const { readerNote } =
        (req.body as { readerNote?: string; assignmentId?: string }) || {};
    ...
    if (attempt.moduleType === "general_goals") {
      ...
      const hasEmptyGoal = goals.some(
        (g: { goalText?: string; rating?: number | null }) =>
          !g.goalText?.trim() || g.rating === null || g.rating === undefined,
      );
      if (hasEmptyGoal) {
        res.status(400).json({
          success: false,
    ...
    if (attempt.moduleType === "weekly_goals") {
      ...
      const hasInvalidRating = goals.some(
        (g) =>
          !isValidRating(g.masteryRating) || !isValidRating(g.pleasureRating),
      );
      if (hasInvalidRating) {
        res.status(400).json({
          success: false,
    ...
    if (attempt.moduleType === "questionnaire") {
      ...
      const got = new Set(
        (attempt.answers || []).map((a) =>
          (a.question as unknown as Types.ObjectId).toString(),
        ),
      );
    ...
    if (attempt.answers?.length) {
      if (attempt.moduleSnapshot?.questions?.length) {
        enrichAnswersWithChoiceMetaFromSnapshot(
          attempt as unknown as Parameters<
            typeof enrichAnswersWithChoiceMetaFromSnapshot
          >[0],
        );
      } else {
        await enrichAnswersWithChoiceMetaFromDB(
          attempt as unknown as Parameters<
            typeof enrichAnswersWithChoiceMetaFromDB
          >[0],
        );
      }
    }
    ...
    const totalScore = (attempt.answers || []).reduce(
      (sum, a) =>
        sum + ((a as unknown as { chosenScore: number }).chosenScore || 0),
      0,
    );
    const band =
  ...
  try {
    ...
    if (status === "active") {
      ...
      const attempts = rows.map((a) => ({
        ...a,
        percentComplete: computePercentCompleteForAttempt(
          a as Parameters<typeof computePercentCompleteForAttempt>[0],
        ),
      }));
...
export const getTherapistLatest = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, rows: decorated, nextCursor, totalCount });
  } catch (error) {
    errorHandler(res, error);
  }
};
...
export const getPatientModuleTimeline = async (req: Request, res: Response) => {
  try {
    ...
    const isOwningTherapist = await therapistCanSeePatient(
      therapistId,
      new Types.ObjectId(patientId),
    );
    const canSee = isAdmin(me) || isOwningTherapist;
    if (!canSee) {
      res.status(403).json({
        success: false,
    ...
    if (isAdmin(me) && !isOwningTherapist) {
      await logAdminAction(req, {
        action: "patient.attemptsViewed",
        resourceType: "patient",
        resourceId: new Types.ObjectId(patientId),
        outcome: "success",
      });
    }
...
export const getMyAttemptDetail = async (req: Request, res: Response) => {
  try {
    ...
    if (attempt.moduleType === "questionnaire") {
      payload.detail = decorateAttemptItems(
        attempt as unknown as Parameters<typeof decorateAttemptItems>[0],
      );
    } else if (attempt.moduleType === "activity_diary") {
      payload.diary = decorateDiaryDetail(
        attempt as unknown as Parameters<typeof decorateDiaryDetail>[0],
      );
    }
...
export const getTherapistAttemptModules = async (
  req: Request,
  res: Response,
) => {
  try {
